...
def install_labels(cls, quiet=True, stdout=None):
    ...
    if not hasattr(cls, "__label__"):
        if not quiet:
            stdout.write(
                " ! Skipping class {0}.{1} is abstract\n".format(
    ...
    for name, property in cls.defined_properties(aliases=False, rels=False).items():
        ...
        if property.index:
            if not quiet:
                stdout.write(
                    " + Creating index {0} on label {1} for class {2}.{3}\n".format(
            ...
            try:
                ...
                if (
                    str(e.message)
                    .lower()
                    .startswith("an equivalent index already exists")
                ):
                    stdout.write("{0}\n".format(str(e)))
                else:
                    raise
            ...
            if not quiet:
                stdout.write(
                    " + Creating unique constraint for {0} on label {1} for class {2}.{3}\n".format(
                        name, cls.__label__, cls.__module__, cls.__name__
                    )
                )
            try:
                ...
                if (
                    str(e.message)
                    .lower()
                    .startswith("an equivalent constraint already exists")
                ):
                    stdout.write("{0}\n".format(str(e)))
                else:
                    raise
